...
int index(struct node* helper,int x, int y)
{
...
int VEB_successor(struct node* helper, int key)
{
    ...
    if (helper->universe_size == 2) {
        ...
        if (cls_max!= -1&&low(helper,key) < cls_max) {
            ...
            return index(helper,high(helper,key), offset);
        }
            ...
            if (succ_cluster == -1) {
                ...
                return index(helper,succ_cluster, offset);
            }
        }
    }
}
...
{
    int n,size;
    printf("Enter the universe set size: ");
    scanf("%d",&size);
    ...
    int x;
    printf("Enter the number whose successor you need to find: ");
    scanf("%d",&x);
